...
TnkAdAnalytics.tnkAdEVentListener = object : TnkAdAnalytics.TnkAdEVentListener {
    override fun onEvent(event: String, params: HashMap<String, String>) {
        when(event) {
            ...
            // 상세화면에서 참여 버튼 클릭  - app id, app name(광고명)
            TnkAdAnalytics.Event.CLICK_JOIN_BUTTON -> Log.d("광고 상세에서 참여 클릭", "onEvent: $event, $params") 
            // 광고 상세 출력  - app id, app name(광고명), **json 광고 데이터(하단의 설명 참조)
            TnkAdAnalytics.EVENT.DETAIL_VIEW_SHOW -> Log.d("광고 클릭", "onEvent: $event, $params")
            // 광고 참여(목록에서 바로 참여 or 상세화면에서 참여 버튼 클릭) - app id, app name(광고명)
